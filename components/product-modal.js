...
document.addEventListener('DOMContentLoaded', function() {
    if (document.querySelector('custom-product-card[modal-id]')) {
        ...
        productModals.forEach(product => {
            ...
            modal.id = modalId;
            modal.className = 'fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4 product-modal';
            modal.innerHTML = `
                <div class="bg-white rounded-lg w-full max-w-2xl overflow-hidden">
                    <div class="relative">
                        <img src="${image}" alt="${title}" class="product-modal-image w-full">
                        <button onclick="document.getElementById('${modalId}').classList.add('hidden')" class="absolute top-4 right-4 bg-white p-2 rounded-full shadow-md text-gray-700 hover:text-pink-500">
                            <i data-feather="x"></i>
                        </button>
                    </div>
                    <div class="p-6">
                        ...
                        <button class="bg-pink-500 hover:bg-pink-600 text-white font-bold py-3 px-6 rounded-full transition duration-300 w-full flex items-center justify-center gap-2" id="whatsappBtn">
            ...
            modal.querySelector('#whatsappBtn')?.addEventListener('click', (e) => {
                e.preventDefault();
